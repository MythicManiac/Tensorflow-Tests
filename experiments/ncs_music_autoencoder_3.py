...
class ExperimentalModel(object):
    ...
    def build(self):
        ...
        encoder.add(MaxPool1D(2))
        encoder.add(Conv1D(1024, 3, activation="relu", padding="same"))
        model.add(encoder)

        model.add(MaxPool1D(2, name="latent"))

        decoder = Sequential(name="decoder")
        decoder.add(Conv1D(1024, 2, activation="relu", padding="same"))
        ...
        decoder.add(UpSampling1D(2))
        decoder.add(Conv1D(128, 2, activation="relu", padding="same"))  # If this doesn't get loss < 20, remove the smallest 2 layers
        decoder.add(UpSampling1D(2))
